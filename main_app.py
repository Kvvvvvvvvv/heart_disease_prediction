...
# API error handler to ensure JSON responses for API routes
...
def api_error_handler(error):
    ...
    if request.path.startswith('/api/'):
        return jsonify({
            'status': 'error',
            'message': str(error.description) if hasattr(error, 'description') else 'An error occurred',
            'data': {}
        }), error.code
    # For non-API routes, return normal error page
...
def get_features():
    ...
    try:
        ...
        return jsonify({
            'status': 'success',
            'message': 'Feature names retrieved successfully',
...
def api_predict():
    # API version of the predict route with proper authentication
    try:
        import joblib
        import json
        import pandas as pd
...
def api_get_features():
    # API version of the features route
    try:
        with open("feature_names.json", "r") as f:
            feature_names = json.load(f)
        return jsonify({
...
# Removed import from original app.py to avoid conflicts with blueprint registration
# Original functionality has been integrated into the blueprint structure
